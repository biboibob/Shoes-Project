...
const OrderList = () => {
  ...
  return (
    <div className="flex flex-col container gap-3 py-3">
      ...
      {!uiSelector.skeleton ? (
        listOrder
          .sort(
            (a, b) =>
              moment(b.purchased_date).unix() - moment(a.purchased_date).unix()
          )
          .map((val, idx) => (
            <div
              className="flex flex-col md:flex-row justify-between bg-white p-3 rounded-lg gap-3 md:gap-0 shadow-sm"
              key={idx}
              ...
              <div className="flex flex-col gap-3">
                ...
                {val.transaction_detail_parent.length > 1 && (
                  <span className="text-xs md:text-sm text-dark-gray-3">
                    + {val.transaction_detail_parent.length - 1} Other Products
                  </span>
                )}
              </div>
